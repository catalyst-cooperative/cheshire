...
`.gitattributes` configures a few pieces of git behavior for specific files:

- `pixi.lock` is marked `linguist-generated=true`, so GitHub collapses it by default
    in PR diffs instead of it dominating the review every time a dependency changes.
- `pixi.lock` is also marked `merge=ours`, so a merge conflict on the generated
    lockfile keeps your side instead of attempting a line-by-line merge -- it gets
    regenerated by `pixi install`/`pixi update` anyway, so a hand-merged version would
    just get replaced.
- `*.ipynb` is marked `linguist-detectable=false`, so notebook JSON doesn't dominate
    GitHub's per-repo language statistics.
- **`merge=ours` requires one-time local setup that can't be done from within the
    repository itself**: run `git config merge.ours.driver true`. Without it, git
    silently ignores the attribute and falls back to a normal conflict-marker merge on
    `pixi.lock`. This has to be configured per clone, on purpose -- a tracked file
    specifying an arbitrary merge driver to run would let anyone who can open a PR
    execute code on a reviewer's machine, so git requires each clone to opt in locally
    rather than trusting a driver defined in the repository.
...
- [typos](https://github.com/crate-ci/typos) (via the `adhtruong/mirrors-typos`
    pre-commit hook) spell checks documentation and code comments. It runs read-only,
    so it flags typos rather than silently rewriting them.
- [detect-secrets](https://github.com/Yelp/detect-secrets) scans changed files for
    accidentally committed secrets and credentials, checking new findings against the
    `.secrets.baseline` file at the root of the repo. If it flags something that isn't
    actually a secret, regenerate the baseline with
    `detect-secrets scan --baseline .secrets.baseline` and commit the update.
